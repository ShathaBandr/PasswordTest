...
function checkPasswordStrength() {
    ...
    if (password.value.length > 12) {
        setColorAndText("red", "Password greater than 12 characters");
        return false;
    }
    ...
    if (password.value.length < 7) {
        ...
        if (
            ...
            (lowerCase && upperCase && specialCharacters) ||
            (lowerCase && numbers && specialCharacters) ||  
...
form.addEventListener("submit", function(e) {
    ...
    if (strengthBar.style.width === "90%" || parseInt(strengthBar.style.width) >= 80) { 
        // التحقق من أن قوة كلمة المرور 100%
        alert("Password accepted. Refreshing the page.");
